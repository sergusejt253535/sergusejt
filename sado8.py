...
def get_live_data():
    ...
    total_vol = 0
    try:
        headers = {'User-Agent': 'SDR_Terminal'}
        ...
        if r.status_code == 200:
            ...
            for item in active_data:
                ...
                if guc > 88: sig, ana = "🛡️ SELL", "🚨 ZİRVE: Kâr Al & Nakde Geç / PEAK: Take Profit & Exit"
                elif guc < 15: sig, ana = "💰 BUY", "🔥 DİP: Kademeli Topla / BOTTOM: Accumulate"
                elif 15 <= guc < 40: sig, ana = "🥷 WAIT", "⌛ PUSU: Bekle ve İzle / AMBUSH: Wait & Watch"
                else: sig, ana = "📈 FOLLOW", "💎 TREND: Takip Et / TREND: Keep Following"
...
with c1:
    st.markdown("""<div class="info-box" style="border-left: 12px solid #ff4b4b;">
        ...
        <p><b>[TR]:</b> Bu panelde sunulan veriler sadece bilgilendirme amaçlıdır. Yatırım danışmanlığı kapsamında değildir. Kripto paralar yüksek risk içerir, tüm sorumluluk kullanıcıya aittir.</p>
        <p><i><b>[EN]:</b> The data presented here is for informational purposes only. It is not investment advice. Cryptocurrencies involve high risk; all responsibilities belong to the user.</i></p>
    </div>""", unsafe_allow_html=True)

with c2:
    st.markdown("""<div class="info-box" style="border-left: 12px solid #FFD700;">
        <h3 style='color:#FFD700;'>🛡️ SDR STRATEJİ / STRATEGY</h3>
        <p><b>[TR]:</b> Güç %88 üzerindeyse zirve noktasına yaklaşılmıştır, kâr alımı düşünülmelidir. %15 altı ise güvenli toplama bölgesidir. Sistem Binance verileriyle her 15 saniyede bir güncellenir.</p>
        <p><i><b>[EN]:</b> If power is above 88%, the peak is near and profit-taking should be considered. Below 15% is the safe accumulation zone. The system updates every 15s via Binance API.</i></p>
    </div>""", unsafe_allow_html=True)
