...
# Hızı 15 saniyeye sabitledik, sistem artık uyumaz
st_autorefresh(interval=15 * 1000, key="sdr_vizyon_final")
...
# --- 4. VERİ MOTORU (SÜREKLİ TAZE VERİ) ---
@st.cache_data(ttl=5) # Hafızayı her 5 saniyede bir zorla boşaltıyoruz
def get_sdr_data(timestamp):
    ...
    assets = ",".join(coins)
    # URL sonuna eklenen t={} parametresi Streamlit'i her seferinde taze veri çekmeye zorlar
    url = f"https://min-api.cryptocompare.com/data/pricemultifull?fsyms={assets}&tsyms=USD&e=Binance&t={timestamp}"
    rows = []
    try:
...
# Her seferinde farklı bir timestamp göndererek cache'i bypass ediyoruz
df = get_sdr_data(int(datetime.now().timestamp()))
...
if not df.empty:
    ...
    # Sağ altta canlı saniye akışı
    st.markdown(f"<p style='color:#00ffcc; font-size:12px; text-align:right;'>🕒 Live Sync: {datetime.now().strftime('%H:%M:%S')}</p>", unsafe_allow_html=True)
...
with inf2:
    st.markdown("""<div class="info-box" style="border-top: 4px solid #00f2ff;">
        ...
        <p style='font-size:13px; color:#ccc;'>
        <b>[TR]:</b> SDR PRESTIGE METODOLOJİSİ; piyasa likiditesini ve fiyat sapmalarını Binance Global verileriyle saniyeler içinde analiz eder.<br><br>
